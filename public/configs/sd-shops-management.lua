return {
    ...
    -- Restrict shop purchases to specific character identifiers (whitelist)
    PurchaseRestrictions = {
        enabled = false,  -- Set to true to only allow whitelisted identifiers to purchase shops
        -- List of allowed character identifiers
    ...
    OwnershipLimits = {
        enabled = false,  -- Set to false to disable ownership limits (unlimited shops)
        maxShops = 1     -- Maximum number of shops one player can own (ignored if enabled = false)
    },

    -- Shop Selling Configuration
    -- Controls whether shop owners can sell their shops back to the server
    SellShop = {
        enabled = true,  -- Set to false to disable selling shops (hides option in Danger Zone)
    },

    -- Shop Transfer Configuration
    -- Controls whether shop owners can transfer ownership to other players
    TransferShop = {
        enabled = true,  -- Set to false to disable transferring shops (hides option in Danger Zone)
        -- Restrict who can receive shop transfers (whitelist)
        restrictions = {
            enabled = false,  -- Set to true to only allow transfers to specific identifiers
            usePurchaseRestrictions = false,  -- If true, uses the same identifiers from PurchaseRestrictions.allowedIdentifiers
            -- Or specify separate identifiers here (only used if usePurchaseRestrictions = false)
            allowedIdentifiers = {
                -- 'XV3T39I3',
                -- 'XYZ98765',
            },
    ...
    -- Available licenses for product restrictions
    -- Shop owners can require customers to have specific licenses to purchase certain products
    -- These licenses must match the license types used by your server's license system
    AvailableLicenses = {
    ...
    -- These overrides appear in both the owner's "Add Product" panel and the customer-facing shop.
    ...
    --
    -- ITEM METADATA:
    -- Any whitelist item can include a 'metadata' table that gets passed to the inventory system on purchase.
    ...
    -- Any whitelist item can include a 'displayMetadata' table to register custom metadata keys for tooltip display.
    ...
    --
    -- This lets shop owners sell different "versions" of the same base item with unique presentation
    -- and metadata. For example, a dispensary could offer different grades of the same product:
    --
    -- Example:
    --   { item = 'water', canOrder = true, orderPrice = 2 },
    --   { item = 'water', canOrder = true, orderPrice = 8, label = 'Premium Spring Water', description = 'Imported artisan spring water.', image = 'water_premium.png', metadata = { quality = 100 }, displayMetadata = { quality = 'Quality' } },
    ProductWhitelist = {
        ['247store'] = {
            -- Food & Drinks
            { item = 'water', canOrder = true, orderPrice = 2 },
            { item = 'water', canOrder = true, orderPrice = 5, label = 'Premium Spring Water', description = 'Imported artisan spring water.', image = 'lockpick.png', metadata = { quality = 100, purified = true }, displayMetadata = { quality = 'Quality', purified = { label = 'Purified', trueValue = 'Yes', falseValue = 'No' } } }, -- Same item, different version
