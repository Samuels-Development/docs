...
-- NOTE: debugs for points are slightly inaccurate, the zone may appear slightly larger than it actually is.
...
-- Settings for forcing peds to stand still when targeted, this is great if you get annoyed at peds walking while you're attempting to press the target.
Config.TargetStandStill = {
    Enable   = true, -- whether targeting makes the ped stand still
    Duration = 4000, -- how long (in ms) they remain standing
}
...
-- Enable ox_inventory metadata support for items (true/false)
-- When enabled, allows selling items with specific metadata (e.g., different weed strains)
Config.EnableMetadata = true -- true/false

-- Encounter settings for special bulk sales.
Config.Encounters = {
    XPPerEncounter = 100,
    Cooldown = { Enable = true, Period = { Hour = 0, Min = 30 } }, -- Cooldown between potential encounters
...
Config.Command = {
    Enable = false, -- Enable/Disable the command
...
-- Animation configuration: defines shared settings and allows per-drug overrides
Config.Animations = {
    -- Shared settings used by all drug-handshake animations
    general = {
        -- Model & attachment offsets for the cash prop, these can be modified and will apply to ALL drugs. Cash stuff can't be changed per-drug.
        cashModel = 'hei_prop_heist_cash_pile',
        cashPos   = { x = 0.13, y = 0.02, z = 0.0 },  -- attachment position relative to hand bone
        cashRot   = { x = -90.0, y = 0.0,  z = 0.0 },  -- attachment rotation relative to hand bone
        -- Default animation dictionary and clip for the handoff
        animDict  = 'mp_common',
        animClip  = 'givetake1_a',
    },

    -- Per-drug overrides: only specify the drugModel, positions, rotations,
    -- and (optionally) custom animDict/animClip if the general defaults don't fit.
    drugs = {
        -- Fallback settings when no override exists for a given drugName
        default = {
            drugModel = 'prop_weed_bottle',
            drugPos   = { x = 0.13, y = 0.02, z = 0.0 },
            drugRot   = { x = -90.0, y = 0.0,  z = 0.0 },
            -- No animDict/animClip here → will use general.animDict & general.animClip
        },

        -- Example override for cocaine baggies:
        --[[
        cokebaggy = {
            drugModel = 'prop_drug_package_02',   -- specific object for cocaine
            drugPos   = { x = 0.10, y = 0.00, z = 0.00 },
            drugRot   = { x = -90.0, y = 0.0,  z = 45.0 },
            -- You can also override the animation if needed:
            -- animDict  = 'mp_arresting',
            -- animClip  = 'a_uncuff',
        },]]
        -- Additional drug-specific overrides can be added below
    },
}

Config.RetrieveCopsTime = 15 -- Interval in minutes for the server to broadcast the number of cops to all connected players.
...
Config.Milestones = {
    Enable = true, -- This key enables or disables the milestone system. Set to true to activate. if false option won't appear
...
Config.Delivery = {
    ...
    Drugs = {
        ...
        ['oxy'] = {
            ...
            Price = 135,
            -- This is an example setup for how you can specifiy metadata item variants (if Config.EnableMetadata is true)
            --[[ SellBaseItem = false, -- Set to false to disable delivering the base item without metadata
            MetadataVariants = { -- Sub-table defining metadata variants for deliveries
                {
                    Label = "High Potency Oxy (Batch A1)",
                    Icon = "fas fa-capsules",
                    Price = 200,
                    Metadata = { potency = "high", batch = "A1" }
                },
                {
                    Label = "Medium Potency Oxy (Batch B2)",
                    Icon = "fas fa-capsules",
                    Price = 150,
                    Metadata = { potency = "medium", batch = "B2" }
                },
                {
                    Label = "Low Potency Oxy (Batch C3)",
                    Icon = "fas fa-capsules",
                    Price = 100,
                    Metadata = { potency = "low", batch = "C3" }
                },
                {
                    Label = "Experimental Oxy (Batch X99)",
                    Icon = "fas fa-flask",
                    Price = 300,
                    Metadata = { potency = "experimental", batch = "X99", lab = "underground" }
                }
            } ]]
        },
        ['meth'] = {
...
Config.Zones = {
    [1] = {
        ...
        --[[  Or, for a polygonal zone, supply an array of corner-points instead, make sure to remove/comment the coords line above:
        Coords = {
        ...
        Size = 250.0, -- Radius of the zone from the coordinates. (only used when Coords is a single point)
        PoliceRequirement = {Enable = false, Amount = 1}, -- Police requirement settings (whether police are required and the number needed)
        ...
        Blip = { -- Blip settings for the zone on the map (if the zone is a polygon, a blip won't appear, if it's a single point, it will)
        ...
        Drugs = { -- List of drugs available for sale in this zone
            ['cokebaggy'] = {
                ...
                -- This is an example setup for how you can specifiy metadata item variants (if Config.EnableMetadata is true)
                --[[ SellBaseItem = true, -- Set to true to allow selling the base item without metadata
                MetadataVariants = { -- Sub-table defining metadata variants for this item
                    {
                        Label = "High Purity Cocaine (85%)",
                        Icon = "fas fa-pills",
                        Price = {Randomize = true, Base = 180, Min = 160, Max = 200},
                        Quantity = {Randomize = true, Base = 8, Min = 5, Max = 12},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { purity = 85 }
                    },
                    {
                        Label = "Medium Purity Cocaine (60%)",
                        Icon = "fas fa-pills",
                        Price = {Randomize = true, Base = 120, Min = 100, Max = 140},
                        Quantity = {Randomize = true, Base = 10, Min = 6, Max = 14},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { purity = 60 }
                    },
                    {
                        Label = "Low Purity Cocaine (40%)",
                        Icon = "fas fa-pills",
                        Price = {Randomize = true, Base = 80, Min = 60, Max = 100},
                        Quantity = {Randomize = true, Base = 12, Min = 8, Max = 16},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { purity = 40 }
                    }
                } ]]
            ...
            ['oxy'] = {
                ...
                LevelRestrict = { Enable = false, Level = 1 }, -- Restrict the sale of this drug to a specific level
                -- This is an example setup for how you can specifiy metadata item variants (if Config.EnableMetadata is true)
                --[[ SellBaseItem = false, -- Set to false to disable selling the base item without metadata
                MetadataVariants = { -- Sub-table defining metadata variants for this item
                    {
                        Label = "High Potency Oxy (Batch A1)",
                        Icon = "fas fa-capsules",
                        Price = {Randomize = true, Base = 200, Min = 180, Max = 220},
                        Quantity = {Randomize = true, Base = 5, Min = 3, Max = 7},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { potency = "high", batch = "A1" }
                    },
                    {
                        Label = "Medium Potency Oxy (Batch B2)",
                        Icon = "fas fa-capsules",
                        Price = {Randomize = true, Base = 150, Min = 130, Max = 170},
                        Quantity = {Randomize = true, Base = 5, Min = 3, Max = 7},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { potency = "medium", batch = "B2" }
                    },
                    ...
                    },
                    {
                        Label = "Experimental Oxy (Batch X99)",
                        Icon = "fas fa-flask",
                        Price = {Randomize = true, Base = 300, Min = 250, Max = 350},
                        Quantity = {Randomize = true, Base = 5, Min = 3, Max = 7},
                        LevelRestrict = { Enable = false, Level = 1 },
                        Metadata = { potency = "experimental", batch = "X99", lab = "underground" }
                    }
    ...
    [2] = {
        ...
        --[[  Or, for a polygonal zone, supply an array of corner-points instead, make sure to remove/comment the coords line above:
        Coords = {
            { x = 135.09, y = 377.45, z = 137.86 },
            { x = 747.06, y = 170.25, z = 118.4 },
            { x = 656.07, y = -77.0, z = 107.22 },
            { x = -132.4, y = -106.74, z = 88.68 }
            -- add more points as needed
        },]]
        Size = 250.0, -- Radius of the zone from the coordinates. (only used when Coords is a single point)
    ...
    [3] = {
        ...
        --[[  Or, for a polygonal zone, supply an array of corner-points instead, make sure to remove/comment the coords line above:
        Coords = {
            { x = -1730.62, y = -1342.07, z = 59.41 },
            { x = -1390.27, y = -990.56, z = 51.64 },
            { x = -1623.61, y = -758.84, z = 47.38 },
            { x = -2006.79, y = -1149.12, z = 42.31 }
            -- add more points as needed
        },]]
        Size = 150.0, -- Radius of the zone from the coordinates. (only used when Coords is a single point)
...
Config.Levels = {
    {
        ...
        Robbery = {Enable = false, Chance = 33}, -- Chance of being robbed during a sale at this level
        ...
        -- '1-1' signifies a 1-to-1 ratio of dirty money conversion (e.g., $1 equals 1 unit of Config.DirtyMoney).
        MoneyType = { Type = 'clean', DirtyType = 'standard' }, -- Type of money to give to the player (clean/dirty) and if dirty the type of dirty money to give (standard/1-1)
