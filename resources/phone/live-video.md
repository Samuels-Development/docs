...
Direct connections use the STUN and TURN settings in `configs/voice.lua`. Public STUN is enough for most pairs of players. Adding TURN (`sd_cf_turn_token_id` and `sd_cf_turn_api_token`) covers the rest, and that one setting serves every WebRTC feature together: video calls, the voice mesh, bodycams and Live. Setting it up takes two convars, covered in [step 5 of the installation guide](./installation#_5-turn-on-video-calls-turn).
