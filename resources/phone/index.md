...
- **For developers**: an extensive [export surface](./exports-server), [first-party events](./events-server) on every lifecycle moment, [custom apps](./custom-apps) so any resource can ship its own phone app (with [ready-made templates](https://github.com/Samuels-Development/sd-phone-app-templates) for plain JS, React and Vue), and a [drop-in lb-phone compatibility layer](./lb-phone-compatibility) covering exports, events, dependencies and lb-phone custom apps, with a one-command data migrator. The [lb-tablet layer](./lb-tablet-compatibility) answers its dispatch and MDT exports the same way.
